...
class AccessControl:
    ...
    def _is_within_time_range(self, role):
    	...
    	if isinstance(time_range, list):  # Multiple time ranges (e.g., for cleaners)
        	return any(current_time >= datetime.strptime(start, '%H:%M:%S').time() and 
                   	current_time <= datetime.strptime(end, '%H:%M:%S').time() 
                   	for start, end in time_range)
    	if time_range:  # Single time range
        	start, end = time_range
        	return current_time >= datetime.strptime(start, '%H:%M:%S').time() and \ current_time <= datetime.strptime(end, '%H:%M:%S').time()
    	return False
    ...
    def _is_room_accessible(self, role, room_type):
    	role_room_map = {
        	...
        	"Emergency Responder": []  # Empty means no NORMAL mode access
    	}
